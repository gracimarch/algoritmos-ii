...
def findPrefix(T: Trie, p: str, n: int):
    ...
    words = []
    _findPrefixImpl(currentNode, p, n-len(p), words)
    print(words)
    

def _findPrefixImpl(currentNode: TrieNode, word: str, n: int, words):
    if n == 0:
        if currentNode.isEndOfWord:
            words.append(word)
        return
    ...
    while current is not None:
        j = current.value
        _findPrefixImpl(j, word + j.key, n-1, words)
...
if __name__ == "__main__":
    def printTrie(T):
        _printTrieImpl(T.root, "", 0)

    def _printTrieImpl(node, word, level):
        if node.isEndOfWord:
            print("  " * level + word + " ✓")
        ...
        while current is not None:
            child = current.value
            print("  " * level + child.key)
            _printTrieImpl(child, word + child.key, level + 1)
            current = current.nextNode
